...
type GroupChatMember struct {
	core.Repo[model.GroupChatMember]
	relation *cache.Relation
}

func NewGroupMember(db *gorm.DB, relation *cache.Relation) *GroupChatMember {
	return &GroupChatMember{Repo: core.NewRepo[model.GroupChatMember](db), relation: relation}
}
...
func (g *GroupChatMember) GetMemberIds(ctx context.Context, groupId int) []int {
	...
	_ = g.Repo.Model(ctx).Select("user_id").
		Where("group_id = ? and is_quit = ?", groupId, model.GroupMemberQuitStatusNo).
		Scan(&ids)
...
func (g *GroupChatMember) GetUserGroupIds(ctx context.Context, uid int) []int {
	var ids []int
	_ = g.Repo.Model(ctx).Where("user_id = ? and is_quit = ?", uid, model.GroupMemberQuitStatusNo).
		Pluck("group_id", &ids)
...
func (g *GroupChatMember) GetMembers(ctx context.Context, groupId int) []*model.MemberItem {
	fields := []string{
	...
	tx.Where("group_chat_members.group_id = ? and group_chat_members.is_quit = ?", groupId, model.GroupMemberQuitStatusNo)
...
func (g *GroupChatMember) CountGroupMemberNum(ids []int) ([]*CountGroupMember, error) {
	...
	err := g.Repo.Model(context.TODO()).Select("group_id,count(*) as count").
		Where("group_id in ? and is_quit = ?", ids, model.GroupMemberQuitStatusNo).
		Group("group_id").
		Scan(&items).Error
	if err != nil {
		return nil, err
...
func (g *GroupChatMember) CheckUserGroup(ids []int, userId int) ([]int, error) {
	...
	err := g.Repo.Model(context.TODO()).Select("group_id").
		Where("group_id in ? and user_id = ? and is_quit = ?", ids, userId, model.GroupMemberQuitStatusNo).
		Scan(&items).Error
	if err != nil {
		return nil, err
	}
